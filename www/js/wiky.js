...
var Wiky = {
  ...
  rules: {
	 ...
	 //  emax: not safe for urls: { rex:/_([^_]+)_/g, tmplt:"<em>$1</em>" }, 
	 //  emax: not safe for urls: { rex:/~([^~]+)~/g, tmplt:"<sub>$1</sub>" },
	 //  emax: not safe for urls: { rex:/\(-(.+?)-\)/g, tmplt:"<del>$1</del>" },
...
{ rex:/\[([^ ,]+)[, ]([^\]]*)\]/g, tmplt:function($0,$1,$2){return Wiky.store("<a onclick=\"openLink(event);\"  href=\""+$1+"\">"+$2+"</a>");}},  // wiki block style uri's ..
       { rex:/(((http(s?))\:\/\/)?[A-Za-z0-9\._\/~\-:]+\.(?:png|jpg|jpeg|gif|bmp))/g, tmplt:function($0,$1,$2){return Wiky.store("<img src=\""+$1+"\" alt=\""+$1+"\"/>");} },  // simple images ..
/* emax fixed this regexp to add , and \_ between 9 and \. */
{ rex:/((mailto\:|javascript\:|(news|file|(ht|f)tp(s?))\:\/\/)[A-Za-z0-9,\.:_\/~%\-+&#?!=()@\x80-\xB5\xB7\xFF]+)/g, tmplt:"<a onclick=\"openLink(event);\" href=\"$1\">$1</a>" }  // simple uri's .. 
     ],
     escapes: [
   ...
   inverse: {
	 ...
	 { rex:/<a onclick=\"openLink(event);\" href=\"([^\"]*)\"[^>]*?>(.*?)<\/a>/mgi, tmplt:function($0,$1,$2){return $1==$2?$1:"["+$1+","+$2+"]";}},
